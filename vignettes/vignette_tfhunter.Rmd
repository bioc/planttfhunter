...
objects containing proteomes as input, and it returns a `SummarizedExperiment`
...
list of 5 `AAStringSet` objects by sampling 100 random genes from the example
data set `gsu` 5 times. We will also create a toy species metadata data frame
...
Great, now we have a list of 5 `AAStringSet` objects. Let's also create a 
...
can execute `get_tf_counts()` as below:
...
Cool, huh? In real-world analyses, if you have TF counts per family in 
multiple species, you can try to find associations between TF counts and
eco-evolutionary aspects of each species (e.g., higher frequencies of
